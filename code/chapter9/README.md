# 下文工程（Context Engineering）
顾名思义，“提示工程”的核心是“如何写出有效提示”，尤其是系统提示。
**系统提示词：你是什么，具备什么能力**
**用户提示词：我要你干什么**
...
就像人类有有限的工作记忆容量一样，LLM 也有一笔“注意力预算”。每新增一个 token，都会消耗这笔预算的一部分，
因此我们**更需要谨慎地筛选哪些 tokens 应该被提供给 LLM**。

## 上下文腐蚀（context rot）
随着上下文窗口中的 tokens 增加，模型从上下文中准确回忆信息的能力反而下降。

源自 LLM 的架构约束。Transformer 让每个 token 能够与上下文中的所有 token 建立关联，理论上形成 级别的两两注意力关系。
