...
def majority_vote_cn(votes: Dict[str, str]) -> tuple[str, int]:
    ...
    vote_counts = Counter(votes.values())  # 统计被投的人
    most_voted = vote_counts.most_common(1)[0]  # 获取票数最多人
...
def calculate_suspicion_score(player_name: str, game_history: List[Dict]) -> float:
    ...
    for event in game_history:
        if event.get("type") == "vote" and event.get("target") == player_name:  # 被投票
            score += 0.3
        elif event.get("type") == "accusation" and event.get("target") == player_name:  # 被指控
            score += 0.2
        elif event.get("type") == "defense" and event.get("player") == player_name:  # 辩解
            score -= 0.1
